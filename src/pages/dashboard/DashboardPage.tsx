...
export const DashboardPage: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    setLoading(true);
    try {
      // 1. Fetch Solicitudes con sus áreas y MONTOS de compras
      ...
      // 2. Fetch Pagos con info de proveedor y solicitud para el listado de vencimientos
      ...
      // 3. Fetch Compras con Rubros para el gráfico de torta
      const { data: comprasRubros, error: rubrosError } = await supabase
        .from('compras')
        .select(`
          monto_total,
          presupuestos!presupuesto_id (
      ...
      (pagos as any[])?.forEach(p => {
        ...
        // Proyección mensual (usando pagos pendientes para el flujo futuro)
        const monthYear = p.fecha_vencimiento.substring(0, 7); // YYYY-MM
        monthlyProjection[monthYear] = (monthlyProjection[monthYear] || 0) + monto;

        // Gasto por área (basado en compromisos pendientes)
        const areaNombre = p.compras?.solicitudes?.areas?.nombre || 'General';
        spendingByArea[areaNombre] = (spendingByArea[areaNombre] || 0) + monto;

        // Lista de vencimientos próximos (limitamos a los próximos 10 para la tabla)
      ...
      (comprasRubros as any[])?.forEach(c => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        {/* Fila inferior: Proyección (Ocupa todo el ancho si querés, o media si hay otra cosa) */}
